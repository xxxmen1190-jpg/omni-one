...
const Chat: React.FC<ChatProps> = ({ sidebarOpen, onToggleSidebar }) => {
  ...
  }, [input, isLoading, messages, addMessage, updateLastMessage, setLoading, setStreaming, setAbortController, setDisplayMode]);
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center py-20 animate-in fade-in duration-700">
              ...
              <p className="text-ink-400 max-w-md">Your AI orchestrator. Ask anything — research, code, reasoning, and more.</p>
            </div>
          )}
          {messages.map((m) => (
            <MessageComponent key={m.id} message={{ ...m, displayMode } as EnhancedChatMessage} />
          ))}
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="relative bg-ink-800/60 border border-ink-700 rounded-2xl focus-within:border-blue-500/50 focus-within:shadow-lg focus-within:shadow-blue-900/10 transition-all">
            <textarea
              ...
              rows={1}
              disabled={isLoading}
              className="w-full bg-transparent border-none rounded-2xl px-5 py-4 pr-14 focus:outline-none resize-none text-ink-100 placeholder-ink-500 disabled:opacity-50 scrollbar-thin"
            />
            <div className="absolute right-3 bottom-3">
              {isStreaming ? (
                ...
                <button onClick={handleSend} disabled={isLoading || !input.trim()} className="w-9 h-9 flex items-center justify-center rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-lg shadow-blue-900/20">
