...
const TransparencyPanel: React.FC<TransparencyPanelProps> = ({ message }) => {
  ...
  return (
    <div className="mt-4 pt-4 border-t border-gray-700 space-y-6 animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          {confidenceScore && (
            <div className="flex items-center gap-2">
              ...
              <div className="w-16 bg-gray-700 rounded-full h-1.5">
                <div 
                  className={`h-1.5 rounded-full ${confidenceScore > 0.8 ? 'bg-green-500' : confidenceScore > 0.5 ? 'bg-yellow-500' : 'bg-red-500'}`}
      ...
      {reasoningTrace && reasoningTrace.steps.length > 0 && (
        <div className="space-y-3">
          <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest">Thinking Process</h4>
      ...
      {/* Execution Timeline / Replay View */}
      {executionTimeline && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest">Execution Flow</h4>
            <button className="text-[10px] text-blue-400 hover:text-blue-300 font-bold uppercase tracking-tighter">
              Replay this answer
            </button>
          </div>
      ...
      {/* Sources Panel */}
      {sourcesPanel && sourcesPanel.totalSources > 0 && (
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {sourcesPanel.documents.map((s, idx) => (
              <div key={idx} className="px-2 py-1 bg-gray-800 rounded border border-gray-700 text-[10px] text-gray-300 flex items-center gap-1.5">
                <span className="text-blue-400">📄</span> {s.title}
              </div>
            ))}
            {sourcesPanel.memories.map((s, idx) => (
              <div key={idx} className="px-2 py-1 bg-purple-900/20 rounded border border-purple-800/30 text-[10px] text-purple-300 flex items-center gap-1.5">
                <span className="text-purple-400">🧠</span> {s.title}
              </div>
            ))}
            {sourcesPanel.tools.map((s, idx) => (
              <div key={idx} className="px-2 py-1 bg-green-900/20 rounded border border-green-800/30 text-[10px] text-green-300 flex items-center gap-1.5">
